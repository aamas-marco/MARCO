...
def run(_run, _config, _log):
    ...
    tmp = "%s_mb_explore_samplingt%d_maxsamples%d_ensemble%d" \
          % (args.prefix, args.sampling_timesteps, args.max_samples,
             args.ensemble)
    tmp = str(args.id) + "_" + tmp + "_beta3" + str(args.beta3)
    args.enviornment_model_directory = tmp
...
# def train_exp_policy(args, exp_runner, buffer_exp, episode_exp, last_exp_train, q):
#     print("********* began training exp policy")
...
#     q.put([exp_runner, buffer_exp, episode_exp])
#     print ("********* finished training exp policy")
...
def run_sequential(args, logger):
    ...
    train_runner = EpisodeRunner(args=args, logger=logger)  # collects traj from unbatched mpe mf env
    test_runner = EpisodeRunner(args=args, logger=logger)  # collects traj from (optional) batched mpe mb env
    data_collector = DataCollector(args=args, logger=logger)
    exp_runner = EpisodeRunner(args=args, logger=logger)  # collects traj from (optional) batched mpe mb env
    ...
    train_runner.mbenv = MpeWithModel(env_path=args.enviornment_model_directory,
                                      reward_type=args.reward_type, ensemble=args.ensemble, beta1=args.beta1,
                                      beta3=args.beta3, exploration=False)
    train_runner.env = train_runner.mbenv

    exp_runner.mbenv = MpeWithModel(env_path=args.enviornment_model_directory,
                                    reward_type=args.reward_type, ensemble=args.ensemble, beta1=args.beta1,
                                    beta3=args.beta3, exploration=True)
    exp_runner.env = exp_runner.mbenv
    ...
    while train_runner.t_env <= args.t_max:
        ...
        if args.save_model and (
                train_runner.t_env - model_save_time >= args.save_model_interval or model_save_time == 0):
        ...
        if train_runner.t_env - env_model_save_time >= args.relearn_interval and dataset_size < args.max_samples:
            ...
            get_new_dataset(dataset, t_env=train_runner.t_env, sampling_timesteps=args.sampling_timesteps,
            ...
            for model_type in ["state_reward"]:
                for i in range(args.ensemble):
                    ...
                    pkwargs = {"epochs": 5000, "learning_rate": 1e-3, "batch_size": 500, "hidden_size": 500,
                               "debug": False,
                               "env_model_directory": args.enviornment_model_directory, "reward_type": args.reward_type,
                               "model_num": i}
                    p = mp.Process(target=train, args=pargs, kwargs=pkwargs)
